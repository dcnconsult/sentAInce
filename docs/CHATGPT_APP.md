...
Run the ChatGPT adapter tests with the optional MCP extra installed (they live beside the canonical
MCP-server tests, outside the deterministic `tests/` lock):

```bash
python -m pip install -e .[dev,mcp]
python -m pytest exocortex/tests/test_chatgpt_mcp.py -q
```

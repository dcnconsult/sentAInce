...
What this actually does, in three small pieces, all reversible:
...
Your project's committed files are not touched. Ignore rules go into `.git/info/exclude`, which never
...
- When a task **verifiably succeeds** (a command exits 0), the route that led there earns a deposit in
  the colony, which is the muscle memory. Reading, retrying, or repeating earns nothing; only verified success
...
- After a few successful tasks of the same kind, you'll start seeing a small **earned-memory block**
  injected at the top of your prompts: the settled route for *that kind of task*, with its trust
...
- **On Windows, PowerShell commands are watched but not yet blocked.** The safety veto's list of
  ...
  way. Nothing is hidden, and this is called out in the README's "honest scope" too.
...
From then on, known lethal command types are refused *before they run*, even if the model was
...
![The body page, one silhouette per project, organs colored by live readings with the rule beside each color](assets/body-page.png)
...
- **Green** means a stated rule met a stated number, never a guess. **Gray** organs are switched off on
  purpose. **Dashed outlines** mean "no data yet", so a fresh project is mostly outlines, and **nothing
  ever fakes green**.
- Click **"why?"** under any project to see the organism *show its work*: the exact steps behind its latest
...
Watching several projects? One file lists them all, see [`ESTATE.md`](ESTATE.md). Want trends over time

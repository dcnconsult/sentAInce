...
def _tool(fn, *, output_schema: dict[str, Any] | None = None):
    ...
    Older SDKs ignore output schemas / annotations via TypeError fallback; the function docstrings still carry
    the read-only boundary, and the implementation remains read-only either way.
    """
    kwargs: dict[str, Any] = {}
    if output_schema is not None:
        kwargs["output_schema"] = output_schema
    try:
        return mcp.tool(annotations={"readOnlyHint": True}, **kwargs)(fn)
    except TypeError:
        try:

...
Control plane (Phase 2 — tweak each repo from the browser):
  - ``GET  /``               -> a small HTML control page (one row per repo; tunable knobs as inputs)
...
def discover_repos(scan_roots: list[Path], registry_path: Path | None,
    ...
    if registry_path is not None and Path(registry_path).is_file():   # central registry (override/extend)
        try:
...
<header><h1>🧬 SentAInce — the organism's control plane</h1>
 <div class=law>A <b>SyncQutrit Research Group</b> product · part of the <b>FreqOS</b> portfolio ·
  tune each repo's organs · <a href="/metrics">/metrics</a> · read the story in Grafana :3000</div>
...
def _make_handler(resolve_repos, read_only: bool = False, token: str = ""):
...
def main() -> int:
    ...
    print(f"[exporter] http://{args.host}:{args.port}  metrics=/metrics  control=/  writes={posture}  "
          f"repos: {found}", flush=True)
    try:
        httpd.serve_forever()

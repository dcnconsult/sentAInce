...
Laws encoded here:
  ...
  * Bootstrap by exploration (dormant by default). The τ-floor deadlocks a fresh wiki (a note can't earn τ
    until used, used until injected, injected until it has τ). When ``explore_budget`` > 0, inject up to
    that many SUB-FLOOR candidates per splice — clearly flagged UNVERIFIED, drawn from the front of the
    proposer's relevance order — so a closed exit-0 chain can award them their first τ. Ant-colony
    exploratory edges, made auditable: exploration is an explicit, bounded, labelled channel, never a
    silent lowering of the floor.
...
def _select(graph: WikiGraph, candidate_node_ids, floor: float, cap: int, budget: int):
    ...
    order) and the sub-floor exploration set (proposer order, doc-ordered)."""
    colony = graph.colony
...
def splice_payload(
    ...
    *,
    ...
    payload, optionally appending up to ``explore`` sub-floor exploratory exons (default: the dormant
    Genome ``declarative.explore_budget``). Returns "" (abstain) when nothing survives and nothing is
    explored."""

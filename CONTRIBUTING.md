...
measurement from material we have never seen, including one that goes against us. If you run the notebook
...
- **Packaging and platforms.** Install ergonomics, OS quirks, CI matrix, wheel hygiene.
...
Please **don't** start in `tests/` or `experiments/` (the 99-test evidence lock, where changing it is
changing what the project has proven), or `sentaince/organism/*` / `vendor/kernel/*` (frozen,
integrity-baselined). If your idea needs a new organ: it ships **dormant with a gauge**, so open a
...
- **What lands well:** additive changes (see "Where new work goes"); a short evidence line saying what you ran
...
- Architecture questions: `docs/ADR.md` (the architecture decision records) and `docs/CLAIMS.md` (the binding evidence
  ledger, and nothing may claim past it) are the ground truth.
